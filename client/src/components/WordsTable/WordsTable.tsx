...
const WordsTable = () => {
	...
	const isRenderButton = () => {
		if (selected.length) {
			return <DeleteButton onClick={onWordsDelete} disabled={isWordsDeleting} />
		}
		return width > 400 ? 'Транскрипция' : 'Транс-крипция'
	}
					...
					<Table stickyHeader area-label='sticky table'>
						<TableHead>
							<TableRow>
								...
								<TableCell align='center'>{isRenderButton()}</TableCell>
							</TableRow>
						</TableHead>
						<TableBody>
							{rows.map((row: IWord) => {
